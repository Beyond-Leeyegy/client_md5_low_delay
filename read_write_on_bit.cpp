...
//template<typename T>
void ReadDataFromBuffer(byte* pBuffer, int nStart, byte btLength, /* out */int& nEnd, /* out */ char& retData)
{
        //顺序读位 
        retData = 0; 
        if (btLength > sizeof(char) * 8)
                return;
        byte btData;
        char tData;
        while (btLength--)
...
void ReadStringFromBuffer(byte* pBuffer, int nStart, int nCount, /* out */int& nEnd, /* out */char* pRetData)
{
	...
	{
		ReadDataFromBuffer(pBuffer, nStart, 8, nStart, content);
		pRetData[nIndex] = content;
	}
	nEnd = nStart;
}
