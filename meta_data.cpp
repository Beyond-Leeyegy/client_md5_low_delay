...
void GetMD5(byte*packet,int & end,string& res){
	log("start GETMD5\r\n");
	char content[33];
        log("end:%d",end);
	ReadStringFromBuffer(packet, end, 8, end, content);
        content[8] = '\0';
	log("after readString:%s\r\n",content);
	
        log("end:%d",end);
	res = content;
}

bool ifSame(string& a,string& b){
	...
	if(strcmp(a_,b_)==0){return true;}
	return false;

}

bool checkMD5(byte* packet,int&end){
	log("debug ingo::try GetMD5");
        string res;
	GetMD5(packet,end,res);
	log("back from GETMD5\r\n");
	int frame_num,frame_size;
	char tag;

	GetFrame_No(packet,frame_size,frame_num,tag);

	log("debug info :: after get frame_no,size:%d,num:%d,type:%c\r\n",frame_size,frame_num,tag);
	...
	string md5_ = md5.toString();	

        md5_.copy(content, md5_.length(), 0);
        *(content + md5_.length()) = '\0';



	log("local md5:%s\r\n",content);
	return ifSame(md5_,res);
...
void GetGameRunFrameBody(byte* packet,Game& game){
	...
	log("debug info:: try check md5\r\n");
	//check MD5	
	if(!checkMD5(packet,end)){
		return;		//drop this frame
	}	

	log("debug info:: end check md5\r\n");
